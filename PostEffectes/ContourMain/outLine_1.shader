        ...
        Pass
            ...
            fixed4 frag (v2f i) : SV_Target
                ...
                for(uint index = 0; index<8; index++){
                  ...
                  maxAlpha = max(maxAlpha, tex2D(_SelectionBuffer_1, sampleUV).a);
                }
                //remove core
                float border = max(0, maxAlpha - tex2D(_SelectionBuffer_1, i.uv).a);
                
                // sample the texture
                fixed4 col = tex2D(_MainTex, i.uv);
                col = lerp(col, _OutlineColor, border * _OutlineColor.a);
                col.a = border;
                return col;
                }
            ENDCG
